...
class BaseController {
    ...
    */
	...
	static async deleteById(req, modelName,options) {
		const reqParam = req.params.id;
		let result;
		try {
			if(options) {
	...
	static async bulkCreate(req, modelName, data) {
		...
		let result;
		try {
			result = await req.app.get('db')[modelName].bulkCreate(list,{ returning: true, individualHooks: true })
			.then((savedResource) => {
				result = _.cloneDeep(savedResource, true)
	...
	static async getByOption(modelName, options) {
		let result;
		try {
			result = await app.get('db')[modelName].findOne(options);
		} catch (err) {
			return Promise.reject(err);
		}
		return result;
	...
	static async getByOptions(modelName, options) {
		let result;
		try {
			result = await app.get('db')[modelName].findAll(options);
		} catch (err) {
			return Promise.reject(err);
		}
		return result;
	...
	static async updateByCustom(modelName, data, options) {
		let result;
		try {
			result = await app.get('db')[modelName]
				.update(data, {
					where: options,
	...
	static async deleteId(modelName,id) {
		let result;
		try {
			result = await app.get('db')[modelName].destroy({
				where: { id },
			})
		} catch (err) {
			return Promise.reject(err);
	...
	static async createRecord(modelName, data) {
		let obj = data;
		let result;
		try {
			result = await app.get('db')[modelName].build(obj).save().then(
				savedResource => Promise.resolve(savedResource),
			);
		} catch (err) {
			return Promise.reject(err);

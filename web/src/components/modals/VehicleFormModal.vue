...
export default {
  ...
  methods:{
    ...
    applyClick: async function(){
      if(this.isUpdate) {
        const ret = await this.updateVehicleGroup();
        if(ret) {
        ...
        this.SET_REGISTER_VEHICLE(this.vehicle).then( async (res) => {
          const backup_group = this.vehicle.group
          this.vehicle = _.cloneDeep(res.data, true)
          this.vehicle.group = backup_group
          await this.updateVehicleGroup(true)
          this.alert( res.message, (ok) =>{
            if(ok) {
    ...
    updateVehicleGroup: async function(new_vehicle){
      let ret = false
      if(this.isUpdate || new_vehicle) {
        const send_data = []
        const vid = this.vehicle.id
        if(!new_vehicle) {      
          if(this.orgData.group !== this.vehicle.group) {
            //delete
            send_data.push({
              cmd:200,
              ...this.orgData.groupset
            })            
          }
        }
        //add
        send_data.push({
          cmd:100,
          id: 0,
          gid: this.vehicle.group,
          vid
        })   
        ret = await this.SET_VEHICLE_GROUP(send_data).then(() => {
          return true
        }).catch((error) => {
          this.alert(error.data.message)
          return false
  ...
  mounted(){
    if(!_.isEmpty(this.vehicledata)){
      ...
      obj.VehicleGroup.map( g => {
        this.vehicle.group = g.gid
        this.vehicle.groupset = g
      })
    }
